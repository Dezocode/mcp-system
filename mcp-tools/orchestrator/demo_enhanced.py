...
async def demo_docker_launch():
    ...
    result = await handle_docker_launch(
        {"platform": "auto", "wait_for_ready": False, "timeout": 30}
    )
    ...
    result = await handle_docker_launch(
        {"platform": "linux", "wait_for_ready": False, "timeout": 30}
    )
    demo_result = json.loads(result[0].text)
    print(f"   Success: {demo_result.get('success')}")
    print(f"   Message: {demo_result.get('message')}")
...
async def demo_platform_resolution():
    ...
    if platform_info["system"] == "Linux":
        print(
            f"SystemCtl Available: {platform_info.get('systemctl_available', 'Unknown')}"
        )
        print(
            f"Docker Daemon Available: {platform_info.get('dockerd_available', 'Unknown')}"
        )
...
async def demo_health_monitoring():
    ...
    # System health
    print("\n1. System Health:")
    result = await handle_health_monitoring({"scope": "system", "detailed": True})
    ...
    # Docker health
    print("\n2. Docker Health:")
    result = await handle_health_monitoring({"scope": "docker", "detailed": True})
    health_data = json.loads(result[0].text)
    ...
    if "docker" in health_data:
        ...
        if "error" in docker_health:
            ...
            print(
                f"   Running Containers: {docker_health.get('containers_running', 'N/A')}"
            )
