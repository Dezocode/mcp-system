...
async def demo_orchestrator():
    ...
    print("\n🔍 2. Environment Detection:")
    env_result = await handle_environment_setup({
        "action": "detect",
        "components": []
    })
    ...
    windows_result = await handle_windows_integration({
    ...
    if integration_status['wsl']['is_wsl']:
        ...
        print(f"   Available Distros: {len(integration_status['wsl']['available_distros'])}")
    else:
    ...
    print(f"   Docker Available: {orchestrator_server.windows_docker.docker_client is not None}")
