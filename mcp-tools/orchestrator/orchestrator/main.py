...
class WindowsDockerIntegration:
    ...
    def __init__(self):
        ...
        self.docker_desktop_enabled = (
            os.getenv("DOCKER_DESKTOP_ENABLED", "true").lower() == "true"
        )
        ...
        if self.is_windows:
            try:
                ...
                if result.returncode == 0:
                    ...
                    wsl_info["available_distros"] = self._parse_wsl_distros(
                        result.stdout
                    )
            except Exception as e:
    ...
    async def execute_docker_command(
        self, command: List[str], use_wsl: bool = False
    ) -> Tuple[int, str, str]:
    ...
    def _check_wsl_docker_integration(self) -> bool:
        ...
        try:
            ...
            if self.docker_client:
                ...
                version_info = self.docker_client.version()
                return (
                    "WSL" in version_info.get("Os", "")
                    or "linux" in version_info.get("Os", "").lower()
                )
            return False
        except Exception:
            return False
...
class CLIResolver:
    ...
    def __init__(self, workspace_root: Path):
        ...
        self.allowed_operations = os.getenv(
            "ALLOWED_DOCKER_OPERATIONS", "start,stop,restart,status,logs,inspect"
        ).split(",")
    ...
    def _setup_routes(self):
        ...
        async def detect_platforms():
            ...
            if self.orchestrator_server.windows_docker.is_windows:
                detection["wsl"] = (
                    self.orchestrator_server.windows_docker.detect_wsl_environment()
                )
                detection["docker_desktop"] = (
                    self.orchestrator_server.windows_docker.get_docker_desktop_status()
                )
...
class OrchestratorServer:
    ...
    def __init__(self):
        ...
        self.web_server_enabled = (
            os.getenv("WEB_SERVER_ENABLED", "false").lower() == "true"
        )
    ...
    def _detect_platform_capabilities(self) -> Dict[str, Any]:
        ...
        if platform_info["system"] == "Windows":
            platform_info["capabilities"].extend(
                ["windows_docker_desktop", "wsl_integration"]
            )
            try:
                platform_info["wsl"] = self.windows_docker.detect_wsl_environment()
                platform_info["docker_desktop"] = (
                    self.windows_docker.get_docker_desktop_status()
                )
            except Exception as e:
        ...
        # macOS-specific detection
            ...
            platform_info["capabilities"].extend(
                ["macos_docker_app", "homebrew_support"]
            )
            try:
                ...
                # Check for Homebrew
                homebrew_path = Path("/opt/homebrew/bin/brew") or Path(
                    "/usr/local/bin/brew"
                )
        ...
        # Linux-specific detection
            ...
            platform_info["capabilities"].extend(
                ["linux_docker_daemon", "systemctl_support"]
            )
            try:
                # Check for systemctl
                result = subprocess.run(
                    ["which", "systemctl"], capture_output=True, timeout=5
                )
                platform_info["systemctl_available"] = result.returncode == 0

                # Check for Docker daemon
                result = subprocess.run(
                    ["which", "dockerd"], capture_output=True, timeout=5
                )
    ...
    def _setup_tools(self):
        ...
        logger.info(
            "Setting up MCP tools with enhanced Docker launch and platform resolution"
        )
        logger.info(
            f"Platform capabilities: {self.platform_info.get('capabilities', [])}"
        )
                ...
                "properties": {
                    ...
                    "options": {
                        ...
                        "properties": {
                            "follow": {
                                "type": "boolean",
                                "description": "Follow logs (for logs operation)",
                            },
                            "all": {
                                "type": "boolean",
                                "description": "Show all containers/images",
                            },
                            "stack": {
                                "type": "string",
                                "description": "Docker stack name (for deploy operation)",
                            },
                        },
                    },
                },
...
async def handle_docker_operation(arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    returncode, stdout, stderr = (
        await orchestrator_server.windows_docker.execute_docker_command(
        ...
        if initial_status.get("running", False):
            ...
            if success and wait_for_ready:
                ...
                while (time.time() - start_time) < timeout and not ready:
                    ...
                    try:
                        status = (
                            orchestrator_server.windows_docker.get_docker_desktop_status()
                        )
...
async def handle_environment_setup(arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    if action == "detect":
        ...
        if orchestrator_server.windows_docker.is_windows:
            detection_results["wsl"] = (
                orchestrator_server.windows_docker.detect_wsl_environment()
            )
            detection_results["docker_desktop"] = (
                orchestrator_server.windows_docker.get_docker_desktop_status()
            )
        ...
        for component in components:
            if component == "directories":
                ...
                if WATCHDOG_AVAILABLE:
                    ...
                    setup_results[component] = (
                        f"Watchdog monitoring started: {monitor_id}"
                    )
                else:
...
async def handle_watchdog_monitoring(arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    result = {"tool": "watchdog_monitoring", "action": action}
    ...
    if action == "start" and path:
        ...
        if monitor_id:
            ...
            result["monitoring_status"] = (
                orchestrator_server.watchdog.get_monitoring_status()
            )
            result["success"] = True
...
async def handle_cli_resolution(arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    if validate_only:
        ...
        is_valid, validation_msg = orchestrator_server.cli_resolver.validate_command(
            command
        )
...
async def handle_windows_integration(arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    result = {"tool": "windows_integration", "action": action}
...
async def handle_health_monitoring(arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    result = {"tool": "health_monitoring", "scope": scope, "detailed": detailed}
    ...
    if (
        scope in ["containers", "all"]
        and orchestrator_server.windows_docker.docker_client
    ):
        # Container health
        try:
            containers = (
                orchestrator_server.windows_docker.docker_client.containers.list(
                    all=True
                )
            )
    ...
    if scope in ["services", "all"]:
        ...
        health_data["services"] = {
            ...
            "windows_docker": {
                "docker_client_available": orchestrator_server.windows_docker.docker_client
                is not None,
    ...
    if orchestrator_server.docker_health_check and detailed:
        try:
            infrastructure_health = (
                orchestrator_server.docker_health_check.get_health_check_endpoint_response()
            )
...
async def handle_deployment_orchestration(
    ...
    if workflow not in workflow_steps and workflow != "custom":
            ...
            if dry_run:
                ...
                try:
                    ...
                    if step == "environment_check":
                        ...
                        env_check = await handle_environment_setup(
                            {"action": "validate"}
                        )
                        ...
                        # Build containers
                        build_result = (
                            await orchestrator_server.cli_resolver.execute_command(
        ...
        result["success"] = all(
            r["status"] in ["completed", "skipped", "skipped (dry run)"]
...
async def main():
    ...
    logger.info(
        f"Capabilities: {orchestrator_server.platform_info.get('capabilities', [])}"
    )

    # Check if web server should run alongside MCP
    if orchestrator_server.web_server_enabled:
        logger.info(
            f"Web API enabled at http://{orchestrator_server.web_server_host}:{orchestrator_server.web_server_port}"
        )
        ...
        await asyncio.gather(run_mcp_server(), run_web_server())
    else:
        ...
        async with stdio_server() as (read_stream, write_stream):
            await orchestrator_server.server.run(
                ...
                write_stream,
                InitializationOptions(
                    server_name="orchestrator", server_version="1.0.0", capabilities={}
                ),
            )
...
@click.option(
    "--enable-web-api",
    is_flag=True,
    help="Enable FastAPI web interface for AI steering",
)
...
@click.option(
    "--platform-info", is_flag=True, help="Show platform detection information"
)
def cli(help_info, enable_web_api, web_host, web_port, platform_info):
    ...
    if help_info:
        click.echo(
            "MCP Orchestrator Server - Enhanced Docker Integration with AI Steering"
        )
