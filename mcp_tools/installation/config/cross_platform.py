...
class CrossPlatformResolver:
    ...
    def _detect_platform(self) -> Dict[str, Any]:
        ...
        info['is_ci'] = any(os.environ.get(var) for var in ['CI', 'CONTINUOUS_INTEGRATION', 'JENKINS', 'TRAVIS'])
    ...
    def _resolve_paths(self) -> Dict[str, Path]:
        ...
        if self.platform_info['is_docker']:
            ...
            if self.platform_info['platform'] == 'windows':
                paths['config'] = Path(os.environ.get('APPDATA', paths['home'] / 'AppData' / 'Roaming')) / 'mcp-system'
            elif self.platform_info['platform'] == 'macos':
                paths['config'] = paths['home'] / 'Library' / 'Application Support' / 'mcp-system'
            else:  # Linux, WSL
                paths['config'] = Path(os.environ.get('XDG_CONFIG_HOME', paths['home'] / '.config')) / 'mcp-system'
            
            # Data directory
            if self.platform_info['platform'] == 'windows':
                paths['data'] = Path(os.environ.get('LOCALAPPDATA', paths['home'] / 'AppData' / 'Local')) / 'mcp-system'
            elif self.platform_info['platform'] == 'macos':
                paths['data'] = paths['home'] / 'Library' / 'Application Support' / 'mcp-system' / 'data'
            else:  # Linux, WSL
                paths['data'] = Path(os.environ.get('XDG_DATA_HOME', paths['home'] / '.local' / 'share')) / 'mcp-system'
            
            # Cache directory
            if self.platform_info['platform'] == 'windows':
                paths['cache'] = Path(os.environ.get('TEMP', '/tmp')) / 'mcp-system-cache'
            elif self.platform_info['platform'] == 'macos':
                paths['cache'] = paths['home'] / 'Library' / 'Caches' / 'mcp-system'
            else:  # Linux, WSL
                paths['cache'] = Path(os.environ.get('XDG_CACHE_HOME', paths['home'] / '.cache')) / 'mcp-system'
            
            # Temp directory
            paths['temp'] = Path(os.environ.get('TMPDIR', os.environ.get('TEMP', '/tmp')))
        
        # MCP-specific paths
        paths['mcp_home'] = paths['config'] / '.mcp-system'
        paths['mcp_bin'] = paths['home'] / 'bin' if self.platform_info['platform'] != 'windows' else paths['home'] / 'Scripts'
    ...
    def _resolve_commands(self) -> Dict[str, str]:
        ...
        # Python command
        if self.platform_info['platform'] == 'windows':
            commands['python'] = self._find_command(['python', 'python3', 'py']) or 'python'
        else:
