...
class MCPTester:
    ...
    async def test_tool_execution(
        self, server_name: str, tool_name: str, args: Dict = None
    ) -> TestResult:
            ...
            if test_type == "tool":
                ...
                if response.status_code == expected_status:
                    return TestResult(
                        ...
                        status="pass",
                        message=(
                            f"HTTP {method} {endpoint} returned {response.status_code}"
                        ),
                        duration=time.time() - start_time,
                    )
                else:
    ...
    async def run_server_tests(
        self, server_name: str, test_suite: ServerTestSuite = None
    ) -> List[TestResult]:
        ...
        if health_result.status != "pass":
            print("  Skipping further tests due to health check failure")
            return results
...
Examples:
  ...
  mcp-test mem0 --report report.json # Save detailed report
    ...
    servers_to_test = (
        list(tester.servers.keys()) if args.server == "all" else [args.server]
    )
