...
async def demo_semantic_catalog():
    ...
    try:
        ...
        if result:
            ...
            print(
                f"✅ Hierarchical protection: {data['config']['hierarchical_protection']}"
            )
            ...
            if "high_resolution_execution" in results:
                ...
                print(
                    f"✅ AST analysis: {exec_data.get('ast_analysis', {}).get('total_nodes', 0)} nodes analyzed"
                )
            ...
            if "semantic_analysis" in results:
                ...
                print(
                    f"✅ Functions analyzed: {semantic_data.get('total_functions', 0)}"
                )
                print(
                    f"✅ Compliance score: {semantic_data.get('compliance_score', 0):.1f}%"
                )
        ...
        if result:
            ...
            if compliance_results:
                print(
                    f"✅ Security compliance: {compliance_results.get('security_compliance', {}).get('score', 0)}%"
                )
                print(
                    f"✅ Quality compliance: {compliance_results.get('quality_compliance', {}).get('score', 0)}%"
                )
                print(
                    f"✅ Documentation compliance: {compliance_results.get('documentation_compliance', {}).get('score', 0)}%"
                )
                print(
                    f"✅ Testing compliance: {compliance_results.get('testing_compliance', {}).get('score', 0)}%"
                )
                print(
                    f"✅ Overall score: {compliance_results.get('overall_compliance_score', 0):.1f}%"
                )
        ...
        if autofix_data.get("status") == "completed":
            ...
            print(
                f"   ⏱️  Execution time: {auto_fix_results.get('execution_time', 0):.2f}s"
            )
        ...
        if claude_data.get("status") == "completed":
            comm_results = claude_data.get("results", {}).get(
                "claude_communication", {}
            )
            print(
                f"   ✅ Communication status: {comm_results.get('communication_status', 'unknown')}"
            )
            print(
                f"   📨 Method used: {comm_results.get('communication_method', 'unknown')}"
            )
            if comm_results.get("payload_sent"):
                payload = comm_results["payload_sent"]
                print(
                    f"   📊 Version keeper issues: {payload.get('version_keeper_issues', {}).get('total_issues', 0)}"
                )
        ...
        if github_data.get("status") == "completed":
            ...
            print(
                f"   ✅ Branch created: {branch_results.get('branch_name', 'unknown')}"
            )
            print(
                f"   📤 GitHub push: {github_push.get('push_status', 'not attempted')}"
            )
        ...
        print(
            f"   • 6 action types: full_review, create_version_branch, diff_analysis, semantic_analysis, compliance_check, auto_fix (NEW)"
        )
