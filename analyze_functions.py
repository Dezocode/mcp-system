...
def extract_functions(filepath):
    ...
    try:
        ...
        return {'functions': [], 'classes': [], 'imports': [], 'lines': 0, 'error': str(e)}
...
report.append(f'Found {len(duplicates)} duplicate function names across {stats["total_files"]} files:\n')
...
if len(main_dups) > 1:
    report.append(f'### ⚠️ Multiple main() functions found ({len(main_dups)} occurrences)')
    ...
    if data['functions'] or data['classes']:
        ...
        if data['functions']:
            ...
            for func in sorted(data['functions'], key=lambda x: x['line']):
                ...
                report.append(f'  - `{func["name"]}({args_str})` at line {func["line"]}')
                if func['decorators']:
...
for imp in sorted(all_imports):
    ...
    elif any(imp.startswith(m) for m in ['os', 'sys', 'ast', 'json', 'pathlib', 'collections', 'subprocess', 'datetime']):
