...
'''
        ...
        for i, deletion in enumerate(deletions, 1):
            print(f"   {i}. {deletion.severity.upper()} deletion at line {deletion.line_number}")
...
def test_fix_rate_calculation():
    ...
    # Calculate rates
    fixes_per_minute = demo_results['fixes_applied'] / (demo_results['total_time_seconds'] / 60)
    ...
    print(f"   Advantage: {(fixes_per_minute/65-1)*100:.0f}% faster + surgical protection")
...
if __name__ == "__main__":
    ...
    print(f"   • {rate_results['fixes_applied']} fixes in {rate_results['total_time_seconds']:.1f}s")
    print(f"   • {protection_results['success_rate']*100:.0f}% surgical restoration accuracy")
