...
Automatically detects development environments and initializes appropriate MCP
configuration
"""
...
class MCPAutoDiscovery:
    ...
    def scan_file_contents(
        ...
        try:
            ...
            if file_path.suffix in [
                ...
                scan_results["file_contents"][str(file_path)] = content[
                    :500
                ]  # First 500 chars
    ...
    def check_running_processes(
        ...
        """Check for running processes indicating development environments"""
        detected_processes = []
    ...
    def is_tool_installed(self, tool: str) -> bool:
        ...
        try:
            # Validate tool name: alphanumeric chars, dashes, underscores only
    ...
    def calculate_environment_scores(self, analysis: Dict[str, Any]):
        ...
        for (
            env_type,
            patterns,
        ) in self.detection_patterns.items():
            score = 0
            max_score = 0
            ...
            if "patterns" in patterns:
                ...
                for pattern in patterns["patterns"]:
                    if any(
                        pattern in f
                        for f in analysis["scan_results"]["files"]
                        + analysis["scan_results"]["directories"]
                    ):
                        score += 5
            ...
            if env_type in analysis["scan_results"]["keywords"]:
                score += min(
                    analysis["scan_results"]["keywords"][env_type],
                    20,
                )
                max_score += 20
    ...
    def generate_suggestions(self, analysis: Dict[str, Any]):
        ...
        if any(
            env in analysis["detected_environments"]
            for env in [
                "python",
                "nodejs",
                "rust",
    ...
    def auto_initialize_project(self, analysis: Dict[str, Any] = None) -> bool:
        """Auto-initialize MCP for current project based on analysis"""
        if analysis is None:
        ...
        for action in analysis["recommended_actions"]:
            if action["priority"] == "high":
                ...
                try:
                    # Replace placeholders in command with validated input
                    project_name = re.sub(
                        r"[^a-zA-Z0-9_-]",
                        "_",
                    ...
                    # Execute command with proper list format for security
                    cmd_parts = command.split()
                    # Validate command starts with known safe commands
                    if not cmd_parts or cmd_parts[0] not in [
                        "mcp-universal",
                        "python3",
                        "pip",
                    ]:
                        print(f"    ❌ Skipped unsafe command: {command}")
                        continue
    ...
    def create_environment_report(self, analysis: Dict[str, Any]) -> str:
        ...
        timestamp = time.strftime(
            "%Y-%m-%d %H:%M:%S", time.localtime(analysis["timestamp"])
        )
        report = f"""
...
def main_auto_discovery():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "analyze":
            ...
            usage = "Usage: auto-discovery-system.py [analyze|auto-init|report] [path]"
            print(usage)
            sys.exit(1)
    else:
