            ...
            for line_num, line in enumerate(lines, 1):
                if re.match(r"^\s*def\s+\w+\s*\(", line):
                    ...
                    if not has_docstring:
                        issue = QualityIssue(
                            severity=self.quality_rules["missing_docstrings"][
                                "severity"
                            ],
...
class GuardrailsSystem:
    ...
    def run_full_validation(self, target_path: str) -> ValidationResult:
        ...
        self.logger.info(
            f"Validation complete. Found {len(security_issues)} security issues, "
    ...
    def _generate_recommendations(
        self, security_issues: List[SecurityIssue], quality_issues: List[QualityIssue]
    ) -> List[str]:
        ...
        if critical_security:
            recommendations.append(
                f"URGENT: Fix {len(critical_security)} critical security issues immediately"
            )

        high_security = [i for i in security_issues if i.severity == "high"]
        if high_security:
            recommendations.append(
                f"Address {len(high_security)} high-priority security issues"
            )
        ...
        if len(quality_issues) > 50:
            recommendations.append(
                "Consider implementing automated code formatting and linting"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="MCP System Guardrails - Security & Quality Validation"
    )
