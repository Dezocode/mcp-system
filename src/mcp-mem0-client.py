...
class MCPMem0Client:
    ...
    async def send_request(self, method: str, params: Optional[Dict[str, Any]] = None) -> Dict:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(f"{self.base_url}/sse", 
                                   json=request,
...
async def main():
    ...
    # Example operations
    print("\n1. Saving a memory...")
    result = await client.save_memory("My favorite programming language is Python. I love its simplicity and readability.")
    print(f"   Result: {result}")
    
    print("\n2. Saving another memory...")
    result = await client.save_memory("I'm working on an MCP server integration for long-term memory storage using Mem0.")
    print(f"   Result: {result}")
