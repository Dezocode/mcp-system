...
class MetricsCollector:
    ...
    def __init__(self, collection_interval: float = 5.0):
        ...
        self.pipeline_metrics: deque = deque(maxlen=1000)  # Last 1000 pipeline snapshots
    ...
    def _collect_system_metrics(self) -> Optional[SystemMetrics]:
        ...
        try:
            ...
            if network_io and self.last_network_io:
                net_sent_mb = (network_io.bytes_sent - self.last_network_io.bytes_sent) / (1024 * 1024)
                net_recv_mb = (network_io.bytes_recv - self.last_network_io.bytes_recv) / (1024 * 1024)
            else:
    ...
    def record_pipeline_metrics(self, session_id: str, metrics_data: Dict[str, Any]):
        ...
        try:
            pipeline_snapshot = PipelineMetrics(
                ...
                throughput_ops_per_second=metrics_data.get("throughput_ops_per_second", 0.0),
    ...
    def _check_performance_thresholds(self, system_metrics: SystemMetrics):
        ...
        # Memory threshold
        if system_metrics.memory_percent > self.performance_thresholds["memory_percent"]:
            alerts.append({
                "type": "memory_high",
    ...
    def get_pipeline_summary(self, session_id: str = None) -> Dict[str, Any]:
        ...
        if session_id:
            relevant_metrics = [m for m in self.pipeline_metrics if m.session_id == session_id]
        
        if not relevant_metrics:
            return {"status": "no_data", "message": f"No metrics for session {session_id}"}
        ...
        avg_quality = sum(m.quality_score for m in relevant_metrics) / len(relevant_metrics)
    ...
    def _is_system_healthy(self, metrics: SystemMetrics) -> bool:
        ...
        return (
            metrics.cpu_percent < self.performance_thresholds["cpu_percent"] and
            metrics.memory_percent < self.performance_thresholds["memory_percent"]
        )
    
    def export_metrics(self, filepath: str, include_system: bool = True, include_pipeline: bool = True):
    ...
    def get_performance_deviation(self) -> Dict[str, float]:
        ...
        return {
            ...
            "memory_deviation": current.memory_percent - self.baseline_metrics["memory_percent"],
            "baseline_age_hours": (time.time() - self.baseline_metrics["timestamp"]) / 3600
        }