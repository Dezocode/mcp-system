...
class MCPServiceRegistry:
    ...
    def register_service(
        ...
        self.logger.info(
            f"Registered service: {name} ({service_type.value}) - ID: {service_id}"
        )
        return service_id
        ...
        if service_id in self.services:
            ...
            for capability in service.capabilities:
                if capability.name in self.capabilities:
                    self.capabilities[capability.name] = [
                        cap
                        for cap in self.capabilities[capability.name]
                        if cap != capability
                    ]
    ...
    def get_service_endpoint(
        self, service_id: str, endpoint_type: str = "main"
    ) -> Optional[str]:
        ...
        for service_id, service in list(self.services.items()):
            try:
                ...
                if time_since_heartbeat > self.service_timeout:
                    self.logger.warning(
                        f"Service {service.name} timed out, marking as unhealthy"
                    )
                            ...
                            async with session.get(health_url, timeout=10) as response:
                                if response.status == 200:
                                    if service.status != ServiceStatus.HEALTHY:
                                        ...
                                        self.logger.info(
                                            f"Service {service.name} health restored"
                                        )
                                else:
                                    service.status = ServiceStatus.DEGRADED
                                    self.logger.warning(
                                        f"Service {service.name} health check failed: {response.status}"
                                    )
                    except Exception as e:
                        service.status = ServiceStatus.UNHEALTHY
                        self.logger.error(
                            f"Service {service.name} health check error: {e}"
                        )
    ...
    def get_registry_status(self) -> Dict[str, Any]:
        ...
        capability_counts = {
            cap: len(providers) for cap, providers in self.capabilities.items()
        }
...
class MCPDiscoveryClient:
    ...
    async def find_service(
        self, capability: str, prefer_local: bool = True
        ...
        if prefer_local:
            local_providers = [
                p for p in providers if p.host in ["localhost", "127.0.0.1"]
            ]
            if local_providers:
    ...
    async def call_service(
        self, service_id: str, endpoint: str, data: Dict[str, Any] = None
