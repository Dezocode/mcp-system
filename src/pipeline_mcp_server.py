...
class PipelineMCPServer:
    """Enhanced Pipeline Integration MCP Server with full v1.0 compliance."""
...
async def handle_list_tools() -> List[Tool]:
    ...
    return [
        Tool(
            ...
            description="Run comprehensive linting scan using Version Keeper with JSON output support",
            inputSchema={
                "type": "object",
                "properties": {
                    "session_id": {
                    ...
                    "target_files": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "Specific files to scan (optional, defaults to all)"
                    }
                },
                "required": []
            }
        ...
        Tool(
            ...
            description="Apply automated fixes using Claude Quality Patcher with configurable limits",
            inputSchema={
                "type": "object",
                "properties": {
                    "session_id": {
        ...
        Tool(
            ...
            description="Execute complete pipeline cycle: scan -> fix -> validate -> report",
            inputSchema={
                "type": "object",
                "properties": {
                    "max_cycles": {
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "session_id": {
                        "type": "string",
                        "description": "Specific session ID (optional, returns all if not provided)"
                    },
                    "include_artifacts": {
                        "type": "boolean",
    ...
    if output_format == "json":
        ...
        if output_file.exists():
            with open(output_file, 'r') as f:
                ...
                session.metrics["total_issues_found"] = result_data.get("summary", {}).get("total_issues", 0)
...
async def handle_quality_patcher_fix(arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    if output_file.exists():
        with open(output_file, 'r') as f:
            result_data = json.load(f)
            session.metrics["fixes_applied"] = result_data.get("summary", {}).get("fixes_applied", 0)
            session.metrics["remaining_issues"] = result_data.get("summary", {}).get("remaining_issues", 0)
...
async def handle_pipeline_run_full(arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    for cycle in range(1, max_cycles + 1):
        ...
        if quality_score >= target_quality:
            logger.info(f"Target quality score {target_quality}% achieved: {quality_score:.1f}%")
            results["success"] = True
            break
...
async def handle_github_workflow_trigger(arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    if wait_for_completion:
        ...
        result["note"] = "Workflow triggered successfully. Use GitHub web interface to monitor progress."
    ...
    if check_schemas:
        ...
        for tool in tools:
            ...
            if schema and "type" in schema and "properties" in schema:
                ...
                schema_check["issues"].append(f"Tool {tool.name} has incomplete inputSchema")
...
async def main():
    ...
    logger.info(f"Tools available: 6")
    logger.info(f"MCP Protocol: v1.0")
