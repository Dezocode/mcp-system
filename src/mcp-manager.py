...
class MCPManager:
            ...
            if dep_config["type"] == "docker":
                ...
                if not result.stdout.strip():
                    ...
                    if start_result.returncode != 0:
                        ...
                        env_args = " ".join([f"-e {k}={v}" for k, v in dep_config.get("env", {}).items()])
                        port_args = " ".join([f"-p {k}:{v}" for k, v in dep_config.get("ports", {}).items()])
                        
                        cmd = f"docker run --name {dep_config['name']} {env_args} {port_args} -d {dep_config['image']}"
    ...
    def start_server(self, server_name: str, foreground: bool = False) -> Dict:
        ...
        if not path.exists():
            return {"success": False, "error": f"Server path not found: {path}"}
...
def main():
    ...
    parser.add_argument("action", choices=["start", "stop", "restart", "status", "send", "list"])
