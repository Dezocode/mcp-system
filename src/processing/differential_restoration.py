...
class DifferentialRestoration:
    ...
    def detect_deletions(self, file_path: Path) -> List[CodeDeletion]:
        ...
        try:
            ...
            differ = difflib.unified_diff(
                baseline_lines, current_lines, lineterm="", n=3  # Context lines
            )
    ...
    def _analyze_deletion_block(
        self, file_path: Path, block: List[str], baseline_lines: List[str]
        ...
        context_after = baseline_lines[
            line_number
            + len(block) : min(len(baseline_lines), line_number + len(block) + 3)
        ]
    ...
    def create_restoration_plan(
        self, deletions: List[CodeDeletion], threshold: float = 0.6
    ) -> RestorationPlan:
        """Create a surgical restoration plan for high-confidence deletions"""

        restorations_needed = [
            d for d in deletions if d.restoration_confidence >= threshold
        ]
        ...
        summary = {
            ...
            "critical_restorations": len(
                [r for r in restorations_needed if r.severity == "critical"]
            ),
            "important_restorations": len(
                [r for r in restorations_needed if r.severity == "important"]
            ),
            "minor_restorations": len(
                [r for r in restorations_needed if r.severity == "minor"]
            ),
        }
    ...
    def _generate_edit_commands(
        self, restorations: List[CodeDeletion]
    ) -> List[Dict[str, Any]]:
                ...
                commands.append(
                    {
                        ...
                        "old_string": (
                            "".join(r.context_after[:1]) if r.context_after else ""
                        ),
                        "new_string": r.deleted_content
                        + ("".join(r.context_after[:1]) if r.context_after else ""),
            ...
            try:
                ...
                if command["tool"] == "Edit":
                    ...
                    new_content = content.replace(
                        command["old_string"], command["new_string"]
                    )
