...
class JobQueue:
    ...
    def __init__(self, max_concurrent_jobs: int = 3):
        ...
        logger.info(
            f"JobQueue initialized with max {max_concurrent_jobs} concurrent jobs"
        )
    ...
    def _job_completed(self, job_id: str, task: asyncio.Task):
        ...
        try:
            ...
            completed_results = [
                r
                for r in self.completed_jobs.values()
                if r.status in ["completed", "failed"]
            ]

            if completed_results:
                self.stats["average_queue_time"] = sum(
                    r.queue_time for r in completed_results
                ) / len(completed_results)
                self.stats["average_execution_time"] = sum(
                    r.execution_time for r in completed_results
                ) / len(completed_results)
    ...
    def clear_completed_jobs(self, keep_last: int = 100):
        ...
        sorted_jobs = sorted(
            self.completed_jobs.items(), key=lambda x: x[1].completed_at, reverse=True
        )
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        efficiency = (
            (avg_execution_time / (avg_queue_time + avg_execution_time) * 100)
            if (avg_queue_time + avg_execution_time) > 0
            else 0
        )
        ...
        return {
            ...
            "current_load": {
                "pending_jobs": len(self.pending_jobs),
                "running_jobs": len(self.running_jobs),
                "queue_utilization": len(self.running_jobs)
