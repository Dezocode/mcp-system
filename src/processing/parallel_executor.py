...
class ParallelExecutor:
    ...
    def __init__(self, max_workers: int = None, cpu_bound_workers: int = None):
        ...
        self.process_executor = concurrent.futures.ProcessPoolExecutor(max_workers=self.cpu_bound_workers)
        ...
        logger.info(f"ParallelExecutor initialized: {self.max_workers} thread workers, {self.cpu_bound_workers} process workers")
    ...
    async def _execute_single_task(self, task: ParallelTask) -> TaskResult:
        ...
        try:
            ...
            task_result.execution_time = task_result.completed_at - task_result.started_at
            
            logger.debug(f"Completed task {task.task_id} in {task_result.execution_time:.2f}s")
    ...
    async def _execute_in_process_pool(self, task: ParallelTask) -> Any:
        ...
        if asyncio.iscoroutinefunction(task.function):
            logger.warning(f"Task {task.task_id}: async function in process pool, falling back to thread pool")
    ...
    def _update_performance_metrics(self, task_count: int, execution_time: float, results: List[TaskResult]):
        ...
        with self._lock:
            ...
            successful_tasks = sum(1 for r in results if r.status == TaskStatus.COMPLETED)
            ...
            tasks_per_second = self.resource_usage["total_tasks_executed"] / total_time if total_time > 0 else 0
...
def create_pipeline_task(task_id: str, operation: str, command: List[str], 
                        cwd: str = None, timeout: float = None) -> ParallelTask:
...
def create_file_task(task_id: str, operation: str, filepath: str, **kwargs) -> ParallelTask:
