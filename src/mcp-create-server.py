...
class MCPServerGenerator:
    ...
    def create_python_fastmcp_template(self, server_name: str, port: int, description: str) -> dict:
...
@asynccontextmanager
async def server_lifespan(server: FastMCP) -> AsyncIterator[{server_name.title().replace("-", "")}Context]:
    """
...
'''
    ...
    def create_typescript_template(self, server_name: str, port: int, description: str) -> dict:
...
async function main(): Promise<void> {{
  try {{
    ...
    app.listen(SERVER_PORT, SERVER_HOST, () => {{
      console.log(`${{SERVER_NAME}} HTTP server running on http://${{SERVER_HOST}}:${{SERVER_PORT}}`);
    }});
...
'''
        ...
        return {
            ...
            ".env.example": f"HOST=localhost\nPORT={port}\n# Add your environment variables here\n",
            "README.md": self._generate_readme(server_name, port, description, "TypeScript"),
            "tests/server.test.ts": test_ts
        }
    
    def create_minimal_python_template(self, server_name: str, port: int, description: str) -> dict:
...
'''
        ...
        return {
            ...
            "README.md": self._generate_readme(server_name, port, description, "Python (Minimal)")
        }
    
    def _generate_readme(self, server_name: str, port: int, description: str, language: str) -> str:
...
'''
    ...
    def create_server(self, name: str, template: str, port: int, 
                     description: str = None, path: str = None) -> bool:
        ...
        print(f"3. Edit .env file with your configuration")
...
Examples:
  ...
  mcp-create-server simple-calc --template minimal-python --port 8057
    ...
    parser.add_argument("--port", "-p", type=int, default=8055,
                       help="Port number (default: 8055)")
    parser.add_argument("--description", "-d", 
                       help="Server description")
    parser.add_argument("--path",
                       help="Custom path (default: ~/mcp-<name>)")
