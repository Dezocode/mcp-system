    ...
    try:
        ...
        try:
            ...
            if "claude_communication" in data.get("results", {}):
                ...
                print(f"✅ Communication status: {comm_data.get('communication_status', 'unknown')}")
        ...
        try:
            ...
            if "version_branch" in data.get("results", {}):
                ...
                print(f"✅ Version: {branch_data.get('current_version', '?')} → {branch_data.get('new_version', '?')}")
                
                if "github_push" in branch_data:
                    github_data = branch_data["github_push"]
                    print(f"✅ GitHub push: {github_data.get('push_status', 'unknown')}")
            else:
