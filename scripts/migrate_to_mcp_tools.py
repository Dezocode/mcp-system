...
class MCPPathMigrator:
    ...
    def migrate_server(
        self, server_name: str, old_path: str, dry_run: bool = False
    ) -> bool:
        ...
        if not old_full_path.exists():
            click.echo(
                f"Warning: Source path does not exist: {old_full_path}", err=True
            )
            return False
            ...
            try:
                ...
                for name, server_config in config.items():
                    if (
                        ...
                        and "path" in server_config
                        and server_config["path"] == old_path
                    ):
...
def main(
    ...
    if not legacy_servers:
        click.echo(
            "✅ No legacy servers found - all servers are using standardized paths"
        )
        return
    ...
    if server:
        ...
        if not dry_run:
            click.echo(
                f"✅ Successfully migrated {success_count}/{len(legacy_servers)} servers"
            )
        else:
            click.echo(
                f"Dry run complete - {success_count} servers ready for migration"
            )
