...
class PathMigrator:
    ...
    def __init__(self, dry_run=False, backup=True):
        ...
        self.patterns = [
            ...
            (
                r'cross_platform.get_path("home") / ',
            ...
            # Linux home paths
            (
                r'cross_platform.get_path("mcp_home") / ',
                'cross_platform.get_path("mcp_home") / ',
            ),
            (r"/home/\w+/", 'cross_platform.get_path("home") / '),
            # Hardcoded command paths
            (
                r'"cross_platform.get_path("home") / mcp"',
                'cross_platform.get_command("mcp")',
            ),
            (
                r'f"{cross_platform.get_command(\"python\")} "',
                'f"{cross_platform.get_command(\\"python\\")} "',
            ),
            (
                r'f"{cross_platform.get_command(\"pip\")} "',
                'f"{cross_platform.get_command(\\"pip\\")} "',
            ),
            # Path separators
    ...
    def fix_file(self, filepath: Path) -> bool:
        ...
        try:
            ...
            if (
                filepath.suffix == ".py"
                and "cross_platform" in content
                and "from src.config.cross_platform import" not in content
            ):
                lines = content.splitlines()
                import_added = False
            ...
            if content != original_content:
                ...
                self.changes.append(
                    {
                        "file": str(filepath),
                        "changes": len(re.findall(r"cross_platform", content))
                        - len(re.findall(r"cross_platform", original_content)),
                    }
                )
                return True
        ...
        for pattern in patterns:
            for filepath in directory.rglob(pattern):
                ...
                if any(
                    ...
                    for skip in [".git", "__pycache__", "venv", ".backup"]
                ):
                    continue
    ...
    def fix_directory(self, directory: Path):
        ...
        print(
            f"{'DRY RUN: ' if self.dry_run else ''}Fixing hardcoded paths in {directory}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Fix hardcoded paths for cross-platform compatibility"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--directory", type=Path, default=Path.cwd(), help="Directory to process"
    )
    parser.add_argument(
        "--scan-only", action="store_true", help="Only scan for issues, do not fix"
    )

    args = parser.parse_args()

    migrator = PathMigrator(
        dry_run=args.dry_run or args.scan_only, backup=not args.no_backup
    )
