            ...
            for server_name, server_config in config.items():
                if isinstance(server_config, dict) and "path" in server_config:
                    if server_config["path"] == old_path:
                        ...
                        updated = True
                        self.logger.info(
                            f"Updated path for {server_name}: {old_path} -> {new_path}"
                        )
    ...
    def standardize_existing_servers(self):
        ...
        for server_path in servers:
            ...
            if not validation["valid"]:
                self.logger.warning(
                    f"Server {server_path.name} missing required files: {validation['missing_required']}"
                )

            if validation["missing_recommended"]:
                self.logger.info(
                    f"Server {server_path.name} missing recommended files: {validation['missing_recommended']}"
                )

    def create_server_template(
        self, server_name: str, template_type: str = "python-official"
    ):
...
class MCPToolsEventHandler(FileSystemEventHandler):
            ...
            if path.parent == self.standardizer.mcp_tools_path:
                ...
                if not path.name.startswith("_"):
                    self.standardizer.logger.info(
                        f"New server directory detected: {path.name}"
                    )
    ...
    def on_moved(self, event):
        ...
        if event.is_directory:
            ...
            if (
                old_path.parent == self.standardizer.mcp_tools_path
                and new_path.parent == self.standardizer.mcp_tools_path
            ):
                # Server directory renamed
                self.standardizer.logger.info(
                    f"Server renamed: {old_path.name} -> {new_path.name}"
                )
                self.standardizer.update_path_references(
                    f"mcp-tools/{old_path.name}", f"mcp-tools/{new_path.name}"
                )
...
def main():
    ...
    parser.add_argument(
        "--template", default="python-official", help="Template type for new server"
    )
