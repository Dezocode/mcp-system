...
🤖 Generated with [Claude Code](https://claude.ai/code)"""
    ...
    def analyze_commits_for_version_type(self) -> str:
        ...
        try:
            ...
            has_feature = any(
                any(keyword in commit.lower() for keyword in ["feat:", "feature:", "minor:"])
                for commit in commits
            )

            has_fix = any(
                any(keyword in commit.lower() for keyword in ["fix:", "patch:", "security:"])
                for commit in commits
            )
...
def main():
    ...
    parser.add_argument("--push-branch", action="store_true", help="Push version branch")
