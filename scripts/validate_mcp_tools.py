...
class MCPToolsValidator:
    ...
    def _validate_main_py(self, main_py_path: Path, result: Dict):
        ...
        try:
            ...
            if "def main(" not in content and "if __name__" not in content:
                result["warnings"].append("main.py missing main function or entry point")
    ...
    def _validate_pyproject(self, pyproject_path: Path, result: Dict):
        ...
        try:
            ...
            try:
                ...
                result["warnings"].append("Cannot validate pyproject.toml - tomllib/tomli not available")
                return
        ...
        try:
            ...
            if 'dependencies' in project:
                ...
                if not has_mcp:
                    result["warnings"].append("pyproject.toml may be missing MCP dependencies")
    ...
    def validate_configuration(self) -> Dict[str, any]:
        ...
        config_result["missing_from_filesystem"] = list(config_servers - filesystem_servers)
        
        if config_result["missing_from_config"] or config_result["missing_from_filesystem"]:
            config_result["valid"] = False
    ...
    def generate_report(self, detailed: bool = False) -> Tuple[bool, str]:
        ...
        for server in servers:
            ...
            if detailed or not result["valid"]:
                ...
                if result["missing_recommended"]:
                    report_lines.append(f"  Missing recommended: {', '.join(result['missing_recommended'])}")
        ...
        if config_result["valid"]:
            ...
            if config_result["missing_from_config"]:
                report_lines.append(f"  Servers not in config: {', '.join(config_result['missing_from_config'])}")
                
            if config_result["missing_from_filesystem"]:
                report_lines.append(f"  Config references missing servers: {', '.join(config_result['missing_from_filesystem'])}")
...
@click.option('--mcp-tools-path', default='mcp-tools', help='Path to mcp-tools directory')
...
def main(server: Optional[str], detailed: bool, mcp_tools_path: str, quiet: bool):
    ...
    if server:
        ...
        if result["missing_recommended"]:
            click.echo(f"Missing recommended: {', '.join(result['missing_recommended'])}")
